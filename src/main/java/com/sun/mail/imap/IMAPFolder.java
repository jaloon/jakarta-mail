...
public class IMAPFolder extends Folder implements UIDFolder, ResponseHandler {
    ...
    private final MailLogger connectionPoolLogger;
	...
	*/
     ...
     * @throws MessagingException if the open fails
     ...
     * @throws SearchException    if mail.imap.throwsearchexception is
    ...
    public synchronized Message[] getMessagesByUID(long[] uids)
        ...
        try {
            synchronized (messageCacheLock) {
                ...
                for (int i = 0; i < uids.length; i++)
                    msgs[i] = uidTable.get(Long.valueOf(uids[i]));
                return msgs;
            }
        } catch (ConnectionException cex) {
            throw new FolderClosedException(this, cex.getMessage());
    ...
    public void handleResponse(Response r) {
        ...
        if (!(r instanceof IMAPResponse)) {
            ...
            logger.fine("UNEXPECTED RESPONSE : " + r);
            return;
        }
     ...
     * @throws FolderClosedException if the folder is closed
     * @throws StoreClosedException  if the store is closed
     */
...
class MessageLiteral implements Literal {
    ...
    private final byte[] buf;        // the buffered message, if not null
